...
현재 프로그램은 DCInside 게시물을 대상으로 다음 단계를 한 번씩 순서대로 실행하는 동기식 배치 파이프라인이다.
...
현재 단계의 책임은 **주제 선정과 사람의 검토를 위한 결과 제공**까지다. 대본, 이미지, 음성, 영상은 아직 만들지 않는다.
...
현재 DCInside 수집기, 규칙 필터, Codex 평가기는 이 모듈의 첫 번째 구현으로 본다. 이후 다른 출처를 추가할 때 기존 DCInside 코드를 일반화하기보다 동일한 출력 계약을 구현하는 새 어댑터를 추가한다.

### 4.2 대본 작성

승인된 `TopicBrief`를 목표 길이와 장르에 맞는 쇼츠 대본으로 변환한다.

- 교체 대상: 설명형, 스토리형, 의견 유도형, 제품 뉴스형 대본 전략
- 책임: 훅, 내레이션, 전개, 마무리, 예상 길이
- 출력: `ScriptPackage`

사실과 의견을 구분하고, 대본의 주요 주장에 원문 근거를 연결한다. 대본 작성기는 새로운 사실을 임의로 확정하지 않는다.
...
정확한 필드는 구현 시 JSON Schema 또는 Python 모델로 확정한다. 최소 의미는 다음과 같다.

| 계약 | 핵심 내용 |
| --- | --- |
| `TopicBrief` | 콘텐츠 프로필, 제목, 출처 URL, 확인된 핵심 내용, 쇼츠 각도, 대상 시청자, 훅 후보, 위험과 불확실성 |
| `ScriptPackage` | 내레이션, 훅, 전개 단위, 마무리, 목표·예상 길이, 주장별 출처 연결, 검토 사항 |
...
2. 현재 결과를 `TopicBrief` 계약으로 표현하고 기존 흐름을 주제 선정 모듈로 감싼다.
3. 대본 작성 모듈과 대본 승인 단계를 추가한다.
4. `Storyboard` 계약과 장면 설계 모듈을 추가한다.
5. 이미지 생성 전략 하나와 수동 또는 단순 렌더링 경로를 연결한다.
6. 음성, 자막, 영상 합성 및 최종 검수를 추가한다.
7. 두 번째 콘텐츠 프로필을 구현해 실제로 모듈 교체가 쉬운지 검증한다.
8. 측정된 병목과 운영 요구에 따라 병렬 처리, 작업 큐, 데이터베이스 또는 서비스 분리를 검토한다.
